       ...
       public override void Bind()
        {
            ...
            this.OnViewModelEvent<MaintenanceRequestStatusUpdated>(evt =>
            {
                ...
                HeaderSection.SubtitleLabel.Text = "Total Checkins: " +ViewModel.Checkins.Count;
            });
            ...
            set.Bind(TicketStatusSection.StatusLabel).For(f => f.Text).To(vm => vm.Request.Status);
         //   set.Bind(TicketStatusSection.LastRevisionLabel).For(f => f.Text).To(vm => vm.La).WithFallback("-");
